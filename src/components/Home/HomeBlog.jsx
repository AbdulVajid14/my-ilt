...
function HomeBlog() {
  ...
  // -----------------------------------------------------------------
  // 1. Fetch blogs (take only the newest 9 – enough for 3 slides)
  // -----------------------------------------------------------------
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_BASE_URL}/News`)
      .then((res) => {
        if (res.data.success) {
          // newest first → slice the first 9 (3 slides × 3 cards)
  ...
  // -----------------------------------------------------------------
  // 3. Chunk blogs into groups of 3 (one slide = one group)
  // -----------------------------------------------------------------
  const chunkSize = 3;
  const slides = [];
    ...
    <section className="bg-white py-12 sm:py-16 px-4">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between">
        ...
        <div className="w-full lg:w-1/3 mb-8 lg:mb-0">
          ...
          <p className="text-sm sm:text-base text-gray-600 mb-6">
            Explore quick tips, guides, and stories to help you learn, grow,
            and build a successful career. From digital marketing strategies to
            tech trends and practical insights, our blogs keep you informed and
          ...
          {loading ? (
            ...
            <>
              ...
              <Swiper
                ...
                spaceBetween={24}
                breakpoints={{
                  // When screen ≥ 640px → 1 column (mobile)
                  640: { slidesPerView: 1 },
                  // When screen ≥ 1024px → show the 3-column grid
