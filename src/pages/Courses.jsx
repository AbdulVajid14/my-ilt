...
const Courses = () => {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_BASE_URL}/courses`)
      .then((res) => res.json())
      .then((response) => {
        if (response.success) {
          // Sort by sequence if available, or adjust based on another field
          const sortedCourses = response.data.sort((a, b) => (a.sequence || 0) - (b.sequence || 0));
  ...
  // Primary courses (category_id: 1), limit to 2
  const primaryCourses = courses
  ...
  // Secondary courses (category_id: 2), limit to 4
  const secondaryCourses = courses
    ...
    .map((c) => ({
      ...
      title: c.name,
      img: c.image,
      description: c.description.length > 100 ? c.description.slice(0, 100) + "..." : c.description,
      modules: c.program_modules.split(".\r\n").filter((s) => s.trim()).length,
      duration: `${c.duration} Months`,
    }));
